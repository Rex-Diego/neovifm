...
- This macOS host provides `/usr/local/bin/sshfs`, `/sbin/umount`, `/usr/bin/unzip`, and `/usr/bin/bsdtar`, but no `fuse-zip` or `archivemount`; ZIP listing preview is therefore available while real mounted ZIP browsing remains an opt-in helper-dependent path.
- Media preview deliberately stops at a safe fallback boundary: image dimensions/format, audio container, and video container metadata are available without graphic terminal protocols. `chafa`/`viu` are not installed, so graphical image/video rendering and audio cover extraction remain deferred rather than faked.
