...
- Media preview deliberately stops at a safe fallback boundary: image dimensions/format, audio container, and video container metadata are available without graphic terminal protocols. `chafa` 1.18.2 is now installed for the future terminal-image fallback, but it is not yet wired into the viewer pipeline; graphical image/video rendering and audio cover extraction remain deferred rather than faked.
